...
from .utilsx import ecbc_coeffs_exact, ecbc_coeffs_quad, impedence_exact, impedence_quad
...
def model_fd(rho, mua, musp, spatial_freq, n_media, n_ext):
...
def model_g1_unnorm(rho, mua, musp, wavelength, bfi, tau, n_media, n_ext):
...
def model_fd_g1_unnorm(rho, mua, musp, wavelength, bfi, tau, spatial_freq, n_media, n_ext):
    ...
    src_k = 2 * pi * n_media * spatial_freq
    k0 = 2 * pi * n_media / wavelength
    k = sqrt((mua + 2 * musp * k0**2 * bfi * tau - 1j * src_k) / D)
    return abs(ecbc_reflectance(rho, k, mua, musp, n_media, n_ext))


def model_g1_norm(rho, mua, musp, wavelength, bfi, tau, n_media, n_ext):
...
def model_g2(rho, mua, musp, wavelength, bfi, tau, beta, n_media, n_ext):
    ...
    parameters:
        ...
        n_ext := External Index of Refraction []
    """
    g1 = model_g1_norm(rho, mua, musp, wavelength, bfi, tau, n_media, n_ext)
    return g2_from_g1(g1, beta)


def model_fd_g1_norm(rho, mua, musp, wavelength, bfi, tau, spatial_freq, n_media, n_ext):
    ...
    src_k = 2 * pi * n_media * spatial_freq
    k0 = 2 * pi * n_media / wavelength
    k_tau = sqrt((mua + 2 * musp * k0**2 * bfi * tau - 1j * src_k) / D)
    k_norm = sqrt(mua / D)
    G1_norm = ecbc_reflectance(rho, k_norm, mua, musp, n_media, n_ext)
...
def model_fd_g2_simplified(
    ...
    src_k = 2 * pi * n_media * spatial_freq
    k0 = 2 * pi * n_media / wavelength
    k_ac = sqrt((mua + 2 * musp * k0**2 * bfi * tau - 1j * src_k) / D)
